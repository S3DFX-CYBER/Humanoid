...
async def generate_embedding(text: str) -> list[float]:
    ...
    if not api_key or api_key == "missing_key":
        logger.warning("[embedding] No Gemini API key provided. Falling back to zero-vector stub.")
    ...
    try:
        ...
        logger.error(f"[embedding] Gemini embed failed: {e}. Falling back to zero-vector stub.")
        return [0.0] * DIMENSIONS
